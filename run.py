...
class vido():
    ...
    def createFilm(self,numF):
        if len(self.cadrs)<self.maxTolerance:
            return
        timestr = now.strftime("%Y-%m-%d-%H-%M-%S-%f")+'--'+str(numF)
        print(timestr+" "+str(len(self.cadrs))+" kadrs. rad="+str(self.radius)+" x="+str(self.currX)+" y="+str(self.currY))
...
##            cv2.imwrite('pics/'+timestr+'.png', frame)
##                   
